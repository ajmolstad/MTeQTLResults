...
for (ll in c(5, 10, 15, 18, 20)) {

  for (j in 1:500) {

    temp <- readRDS(paste("rhoY_50_R2_10_eQTLs_", ll, "_", j, ".RDS", sep=""))
...
df2 <- data_summary(dat, varname="R2", 
                    groupnames=c("Method", "rho"))
p1 <- ggplot(df2, aes(x=as.factor(rho), y=R2, group=Method, color=Method, linetype=Method)) +
  ylab(expression(paste("Average test set ", R^2, sep=""))) + 
  geom_line(position=position_dodge(0.2), size=.75) + xlab("Proportion of shared eQTLs") + 
  ...
  geom_errorbar(aes(ymin=R2-2*se, ymax=R2+2*se), width=0, linetype="solid", 
   ...
   theme_bw() + scale_color_viridis(discrete=TRUE, option="A", begin=0, end=.8)
...
p2 <- ggplot(df2, aes(x=as.factor(rho), y=TPR, group=Method, color=Method, linetype=Method)) + 
  geom_line(position=position_dodge(0.2), size=.75) + ylab("LD-adjusted true positive rate") + 
  geom_point(position=position_dodge(0.2), show.legend=FALSE)+ xlab("Proportion of shared eQTLs") + 
...
p3 <- ggplot(df2, aes(x=as.factor(rho), y=FNR, group=Method, color=Method, linetype=Method)) + 
...
pdf("/Volumes/blue/amolstad/amolstad/MTeQTL/Simulations/eQTL_Simulations.pdf", width=11, height=3.2)
...
dat <- data.frame( 
  "Timing" = ours.timing,
  "rho" = rho)
mat1 <- matrix(unlist(tapply(dat$Timing/60, as.factor(dat$rho), summary)), nrow=5, byrow=TRUE)
